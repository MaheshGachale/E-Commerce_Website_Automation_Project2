...
```ts
   ...
   93 |   await page.getByRole('link', { name: 'Samsung galaxy s6' }).click();
   94 |   await page.waitForTimeout(2000);
   95 |   const product2Price = parseInt((await page.getByRole('heading', { name: /\$360/ }).textContent()).replace('$', ''));
   ...
   99 |   await page.getByRole('link', { name: 'Cart', exact: true }).click();
